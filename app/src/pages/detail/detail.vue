...
type Refund = { id: number; refundNo: string; amountCents: number; createdAt: string }
...
const accountName = (accountId?: number) => ledger.state.accounts.find(account => account.id === accountId)?.name || '历史账户'
...
const accountText = computed(() => {
  ...
  const name = (accountId?: number) => ledger.state.accounts.find(account => account.id === accountId)?.name || (accountId ? `账户 ${accountId}` : '—')
...
onLoad(async query => { id.value = Number(query?.id || 0); if (id.value) await load() })
...
async function refund() {
  ...
  saving.value = true
  try {
    const amount = cents(refundAmount.value || String((detail.value.effectiveCents / 100).toFixed(2)))
...
function removeRefund(refundId: number) {
  ...
  <view class="page detail-page">
    ...
    <template v-else-if="detail">
      <view :class="['detail-hero', detail.transaction.type.toLowerCase()]">
        ...
        <view class="detail-amount"><text class="currency">{{ detail.transaction.type === 'EXPENSE' ? '− ' : detail.transaction.type === 'INCOME' ? '＋ ' : '' }}¥</text>{{ yuan(detail.transaction.originalAmountCents).slice(1) }}</view>
        <view :class="['detail-status', { refunded: detail.refundedCents }]"><text class="status-dot" />{{ detail.effectiveCents === 0 ? '已退款' : detail.refundedCents ? '部分退款' : '已完成' }}<text v-if="detail.refundedCents"> · {{ yuan(detail.refundedCents) }}</text></view>
      ...
      <view v-if="detail.refunds.length" class="detail-section"><view class="detail-section-heading"><text class="section-title">退款记录</text><text class="section-meta">{{ detail.refunds.length }} 笔</text></view><view class="detail-info-card"><view class="detail-info-row"><text class="detail-info-label">累计退款</text><text class="liability">{{ yuan(detail.refundedCents) }}</text></view><view v-for="(record, index) in detail.refunds" :key="record.id" class="refund-record-item"><text class="field-icon">↩</text><view class="account-main"><text class="account-name">第 {{ index + 1 }} 笔退款</text><text class="account-desc">{{ record.createdAt.replace('T', ' ').slice(0, 16) }}</text></view><text class="liability">{{ yuan(record.amountCents) }}</text><button class="refund-delete" @click="removeRefund(record.id)">删除</button></view></view></view>
