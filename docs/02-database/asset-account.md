...
`asset_account` 是资产页资金账户和信贷账户的统一主表。账户是可独立新增、编辑、逻辑删除和查询的业务对象，因此使用完整的 10 个公共审计字段，而不是关联表字段模型；当前没有独立的停用状态列。
...
3. 当前交易表和交易 Service 已落地，账务写入会在同一事务中维护余额/欠款；但账户编辑接口仍可直接改余额或欠款，当前不会生成“余额补齐”流水，这是已知产品与数据追溯缺口，不能在文档中描述成已实现。
